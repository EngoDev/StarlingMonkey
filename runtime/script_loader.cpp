...
static std::string resolve_path(std::string_view path, std::string_view base) {
  ...
  while (path_cur_idx < path_len) {
    ...
    if (path_cur_idx - path_from_idx == 2 && path[path_from_idx] == '.' &&
    ...
    // Copy segment equivalent to: strncpy(resolved_path + resolved_len, path + path_from_idx,
    // path_cur_idx - path_from_idx);
...
static ScriptLoader *script_loader(JSContext *cx) { return api::Engine::get(cx)->script_loader(); }
...
static JSObject *get_module(JSContext *cx, JS::SourceText<mozilla::Utf8Unit> &source,
  ...
  RootedString resolved_path_str(cx,
                                 JS_NewStringCopyN(cx, resolved_path.data(), resolved_path.size()));
  ...
  if (!MapSet(cx, script_loader(cx)->module_registry(), resolved_path_val, module_val)) {
    return nullptr;
  }
...
static JSObject *get_module(JSContext *cx, std::string_view specifier,
  ...
  RootedValue resolved_path_val(cx, StringValue(resolved_path_str));
  auto *loader = script_loader(cx);
  RootedValue module_val(cx);
  if (!JS::MapGet(cx, loader->module_registry(), resolved_path_val, &module_val)) {
    return nullptr;
  }
  ...
  if (!loader->load_resolved_script(cx, specifier, resolved_path, source)) {
    return nullptr;
  }
...
static JSObject *get_builtin_module(JSContext *cx, HandleValue id, HandleObject builtin) {
  ...
  if (!JS::MapGet(cx, loader->module_registry(), id, &module_val)) {
    return nullptr;
  }
  if (!module_val.isUndefined()) {
    return &module_val.toObject();
  ...
  JS::CompileOptions opts(cx, loader->compile_options());
  ...
  if (!MapSet(cx, loader->module_registry(), id, module_val)) {
    return nullptr;
  }
...
JSObject *module_resolve_hook(JSContext *cx, HandleValue referencingPrivate,
  ...
  if (!MapGet(cx, loader->builtin_modules(), path_val, &builtin_val)) {
    return nullptr;
  }
  ...
  JS::CompileOptions opts(cx, loader->compile_options());
  auto stripped = strip_prefix(resolved_path, loader->path_prefix());
...
bool module_metadata_hook(JSContext *cx, HandleValue referencingPrivate, HandleObject metaObject) {
  ...
  RootedValue builtin_val(cx);
  if (!MapGet(cx, loader->builtin_modules(), parent_id_val, &builtin_val)) {
    return false;
  }
  if (builtin_val.isUndefined()) {
    return false;
...
ScriptLoader::ScriptLoader(api::Engine *engine, JS::CompileOptions *opts,
                           mozilla::Maybe<std::string> path_prefix)
    : engine_(engine), compile_opts_(opts), path_prefix_(std::move(path_prefix)) {
  JSContext *cx = engine->cx();
  module_registry_.init(cx, JS::NewMapObject(cx));
  builtin_modules_.init(cx, JS::NewMapObject(cx));
  MOZ_RELEASE_ASSERT(module_registry_);
  MOZ_RELEASE_ASSERT(builtin_modules_);
  JSRuntime *rt = JS_GetRuntime(cx);
  SetModuleResolveHook(rt, module_resolve_hook);
  SetModuleMetadataHook(rt, module_metadata_hook);
}

api::Engine *ScriptLoader::engine() const { return engine_; }
HandleObject ScriptLoader::module_registry() { return module_registry_; }
HandleObject ScriptLoader::builtin_modules() { return builtin_modules_; }
const JS::CompileOptions &ScriptLoader::compile_options() const { return *compile_opts_; }
const mozilla::Maybe<std::string> &ScriptLoader::path_prefix() const { return path_prefix_; }

bool ScriptLoader::define_builtin_module(const char *id, HandleValue builtin) {
  JSContext *cx = engine_->cx();
  ...
  if (!MapHas(cx, builtin_modules_, id_val, &already_exists)) {
    return false;
  }
  if (already_exists) {
    return api::throw_error(cx, ScriptLoaderErrors::BuiltinModuleExists, "id");
  }
  if (!MapSet(cx, builtin_modules_, id_val, builtin)) {
    return false;
  }
  return true;
}

void ScriptLoader::enable_module_mode(bool enable) { module_mode_ = enable; }
...
bool ScriptLoader::load_script(JSContext *cx, std::string_view path,
  ...
  if (base_path_.empty()) {
    ...
    resolved = resolve_path(path, base_path_);
  }
...
bool ScriptLoader::eval_top_level_script(std::string_view path,
  ...
  JS::CompileOptions opts(cx, *compile_opts_);
  auto stripped = strip_prefix(path, path_prefix_);
  opts.setFileAndLine(stripped.c_str(), 1);
  ...
  if (engine_->state() == api::EngineState::ScriptPreInitializing) {
